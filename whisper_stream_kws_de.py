...
class Settings:
    ...
    WORDS_FILE = "words_de.txt" # German keywords file
    FUZZY_THRESHOLD = 75
    ...
    # YAMNet Settings (Language-independent acoustic event detection)
    YAMNET_MODEL_HANDLE = 'https://tfhub.dev/google/yamnet/1'
    YAMNET_SCORE_THRESHOLD = 0.3 # Lowered to 0.3 for higher sensitivity
    
    # AudioSet classes for distress/panic (The names are English, as YAMNet is trained this way)
    DISTRESS_SOUNDS = [
...
def run_yamnet_distress_detection(audio_data: np.ndarray, model, class_names: List[str], score_threshold: float) -> List[str]:
    ...
    with tf.device('/cpu:0'):
        ...
        # Aggregation change: Use MAX instead of MEAN to detect short, peak distress sounds
...
class KeywordPublisher(Node):
    def __init__(self, topic_name):
        ...
        # Whisper is correctly loaded to CPU
        ...
        self.get_logger().info(f"Loaded {len(self.keywords)} German keywords and YAMNet.")
        threading.Thread(target=self.process_loop, daemon=True).start()

    def _load_words(self):
        path = Path(self.settings.WORDS_FILE)
        if not path.exists():
            self.get_logger().warn(f" Words file not found: {path}. Using empty keyword list.")
            return []
    ...
    def _process_speech_buffer(self):
        ...
        # --- 1. NON-VERBAL DISTRESS DETECTION (YAMNet) ---
        try:
